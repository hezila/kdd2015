...
class SimpleFeatureBag(FeatureBag):
    ...
    def extract_request_day(self):
        fst_m = int(self.start_day.strftime('%m'))
        if fst_m in [6, 7, 12, 1]:
        ...
        if fst_m in [6, 7, 12, 1] and lst_m in [6, 7, 12, 1]:
    ...
    def extract_duration_days(self):
        ...
        if duration > 19:
            self.feature_keys.append('duration_1month')
            self.feature_values.append(1.0)
        else:
            self.feature_keys.append('duration_1month')
            self.feature_values.append(0.0)
    ...
    def extract_access_count(self):
        ...
        self.feature_keys.append('request_module_count')
    ...
    def extract_access_count_lst2week(self):
        ...
        for d, vs in daily_access.items():
            for v in vs:
                if v not in modules:
                    modules.append(v)
        self.feature_keys.append('module_count_lst2week')
        self.feature_values.append(len(modules) + 0.0)
        ...
        if len(modules) > 0:
            avg = sum(modules) / (len(modules) + 0.0)
        self.feature_keys.append('module_per_day_lst2week')
        self.feature_values.append(avg)
    ...
    def extract_access_count_lst1week(self):
        start = self.end_day - datetime.timedelta(days=7)
        requests = [(log['time'].strftime('%Y%m%d'), log['object']) for log in self.logs if (log['event'] == 'access' and log['time'] >= start)]
        self.feature_keys.append('request_module_count_lst1week')
        self.feature_values.append(len(requests) + 0.0)
        ...
        for d, vs in daily_access.items():
            for v in vs:
                if v not in modules:
                    modules.append(v)
        self.feature_keys.append('module_count_lst1week')
        self.feature_values.append(len(modules) + 0.0)
        ...
        if len(modules) > 0:
            avg = sum(modules) / (len(modules) + 0.0)
        self.feature_keys.append('module_per_day_lst1week')
        self.feature_values.append(avg)
    ...
    def extract_video_count(self):
        ...
        self.feature_keys.append('video_per_day')
        self.feature_values.append(avg)

        return self


    def extract_video_count_lst2week(self):
        requests = [(log['time'].strftime('%Y%m%d'), log['object']) for log in self.logs if log['event'] == 'video']
        self.feature_keys.append('video_request_count_lst2week')
        self.feature_values.append(len(requests) + 0.0)
        ...
        for d, vs in daily_videos.items():
            for v in vs:
                if v not in videos:
                    videos.append(v)
        self.feature_keys.append('video_count_lst2week')
        self.feature_values.append(len(videos) + 0.0)
        ...
        if len(videos) > 0:
            avg = sum(videos) / (len(videos) + 0.0)
        self.feature_keys.append('video_per_day_lst2week')
        self.feature_values.append(avg)
    ...
    def extract_problem_count(self):
        ...
        self.feature_keys.append('problem_per_day')
        self.feature_values.append(avg)
        return self
    ...
    def extract_session_count(self):
        ...
        for k, v in daily_timeline.items():
            ...
            for i in range(len(v) - 1):
                if (v[i+1] - v[i]).seconds >= 60 * 60:
                    cnt += 1.0
            cnt += 1.0
            total_count += cnt
            sessions.append(cnt)
        self.feature_keys.append('session_count')
        self.feature_values.append(total_count)

        return self

    def extract_session_lst2week(self):
        start = self.end_day - datetime.timedelta(days=14)
        ...
        for k, v in daily_timeline.items():
            ...
            total_count += cnt
            sessions.append(cnt)
        self.feature_keys.append('session_count_lst2week')
        self.feature_values.append(total_count)

        return self

    def extract_session_lst1week(self):
        start = self.end_day - datetime.timedelta(days=7)
        request_dates = [(log['time'].strftime('%Y%m%d'), log['time']) for log in self.logs if log['time'] >= start]
        request_dates = sorted(request_dates, key=lambda x: x[1])
        daily_timeline = {}
        ...
        for k, v in daily_timeline.items():
            ...
            total_count += cnt
            sessions.append(cnt)
        self.feature_keys.append('session_count_lst1week')
        self.feature_values.append(total_count)
    ...
    def extract_session_per_day(self):
        request_dates = [(log['time'].strftime('%Y%m%d'), log['time']) for log in self.logs]
        request_dates = sorted(request_dates, key=lambda x: x[1])
        daily_timeline = {}
        for d in request_dates:
        ...
        for k, v in daily_timeline.items():
            ...
            total_count += cnt
            sessions.append(cnt)
        self.feature_keys.append('session_per_day')
    ...
    def extract_request_lag_max(self):
        ...
        if len(request_dates) == 1:
            ...
            lags = [(request_dates[i+1] - request_dates[i]).days for i in range(len(request_dates) - 1)]
        self.feature_keys.append('request_lag_max')
    ...
    def extract_request_lag_mean(self):
        ...
        if len(request_dates) == 1:
            ...
            lags = [(request_dates[i+1] - request_dates[i]).days for i in range(len(request_dates) - 1)]
        self.feature_keys.append('request_lag_mean')
        self.feature_values.append(sum(lags) / (len(lags) + 0.0)) # avoid numpy to use pypy
        return self

    def extract_request_lag_std(self):
        request_dates = sorted(list(set([log['time'].strftime('%Y%m%d') for log in self.logs])))
        request_dates = [datetime.datetime.strptime(d, '%Y%m%d') for d in request_dates]
        lags = []
        ...
        if len(request_dates) >= 5:
            mean = sum(lags) / (len(lags) + 0.0)
            var = 1.0 / (len(lags)) * sum([(l - mean)**2 for l in lags])
            std = math.sqrt(var)
        elif len(request_dates) >= 2:
            max_lag = max(lags)
            min_lag = min(lags)
            std = max_lag - min_lag + 1.0
        else:
            std = 3.0


        self.feature_keys.append('request_lag_std')
        self.feature_values.append(std)
        return self
