...
from week_module_feature_bag import WeekModuleFeatureBag
...
class WeekModuleFeatureExtractor(FeatureExtractor):
    ...
    def _extract_enrollment_features(self, iter):
        for bag in iter:
            for sub_bag in bag.gen_sub_bags():
                yield sub_bag.extract_module_freq_mean(self.module_db)\
    ...
    def _bag_generator(self, iter):
        for k, g in iter:
            yield WeekModuleFeatureBag(k, None, [t[1] for t in g], [], [])
